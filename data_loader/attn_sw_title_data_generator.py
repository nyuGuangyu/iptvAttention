...
class DataGenerator:
    def __init__(self, config):
        ...
        # No normalization for item_attn

        # v = self.x_train_nsw
        # v_min = self.x_train_nsw.min()*1.
        # v_max = self.x_train_nsw.max()*1.
        # v_min = self.x_train_nsw.min(axis=2, keepdims=True)
        # v_max = self.x_train_nsw.max(axis=2, keepdims=True)
        # self.x_train_nsw = (v - v_min) / (v_max - v_min)
        # where_are_NaNs = np.isnan(self.x_train_nsw)
        # self.x_train_nsw[where_are_NaNs] = 0.

        # v = self.x_test_nsw
        # v_min = self.x_test_nsw.min()*1.
        # v_max = self.x_test_nsw.max()*1.
        # v_min = self.x_test_nsw.min(axis=2, keepdims=True)
        # v_max = self.x_test_nsw.max(axis=2, keepdims=True)
        # self.x_test_nsw = (v - v_min) / (v_max - v_min)
        # where_are_NaNs = np.isnan(self.x_test_nsw)
        # self.x_test_nsw[where_are_NaNs] = 0.
